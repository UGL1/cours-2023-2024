...
Le mot-clé \mintinline{python}{assert} sert à vérifier qu'une \textit{assertion} (un test de condition) est vraie. Si c'est le cas le programme continue, sinon une exception du type \mintinline{python}{AssertionError} est levée. Ainsi on peut la gérer avec une structure \mintinline{python}{try} / \mintinline{python}{except} comme vu précédemment.
