...
\end{enumerate}
On peut reprendre la fonction \texttt{dfs\_iterative}, enlever la variable \texttt{visited} qui n'a plus d'intérêt et la remplacer par un dictionnaire nommé \texttt{predecessors}, initialisé avec \mintinline{python}{predecessors[start]=None}.\\
...
Si le sommet courant est \texttt{end} alors on se sert de \texttt{predecessors} pour reconstruire la liste des sommets partant de \texttt{start} (unique sommet qui a pour prédécesseur \mintinline{python}{None}) à \texttt{end}.\\
...
\begin{exercice}
Reprendre l'exercice précédent et coder \mintinline{python}{bfs_path}.
\end{exercice}
\end{document}